...
const ShopContextProvider = (props) => {
    ...
    useEffect(() => {
        ...
        if (localStorage.getItem('auth-token')) {
            fetch('http://3.107.70.18:4000/getcart', {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                setCartItems(data);
                fetch('http://3.107.70.18:4000/getAddress', {
                    method: 'POST',
                    headers: {
                        Accept: 'application/json',
                        'auth-token': localStorage.getItem('auth-token'),
    ...
    const addToCart = (ItemId, size) => {
        ...
        if (localStorage.getItem('auth-token')) {
            fetch('http://3.107.70.18:4000/addtocart', {
                method: 'POST',
                headers: {
                    Accept: 'application/json',
                    'auth-token': localStorage.getItem('auth-token'),
    ...
    const removeFromCart = (ItemId, size) => {
        ...
        if (localStorage.getItem('auth-token')) {
            fetch('http://3.107.70.18:4000/removefromcart', {
                method: 'POST',
                headers: {
                    Accept: 'application/json',
                    'auth-token': `${localStorage.getItem('auth-token')}`,
